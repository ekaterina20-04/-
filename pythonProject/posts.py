        ...
        if article:
            if draft_access or current_user.id == article.user_id:
                if request.method == "POST":
                    ...
                    git_check = save_changes(request.form['text'],fr"C:\Users\Professional\PycharmProjects\pythonProject\articles\article_{id}")
                    if git_check:
...
def save_changes(edit_text,file_path):
    try:
        ...
        repo.index.commit(f"Date: {formatted_datetime} Edited file: {file_path}")
        repo.index.update()
        return True
    except:
        return False
        ...
        if msg[msg.find('Edited file: ') + 13:] == file_path:
            ...
            for file in files:
                if file.path == file_path[file_path.rfind('\\') + 1:]:
                    ...
                    date = msg[:msg.find('Edited file: ')]
                    all_commits[date] = file_content
    return all_commits